...
from .hidroelectrica_const import (
    API_BASE,
    API_TIMEOUT,
    DEFAULT_LANGUAGE,
    ENDPOINT_GET_BILL,
...
class EroareApiHidroelectrica(Exception):
    """Eroare generică aruncată de API client."""


class EroareAutentificareHidroelectrica(EroareApiHidroelectrica):
    """Eroare de autentificare (credențiale invalide)."""


class ClientApiHidroelectrica:
    ...
    async def async_login(self) -> bool:
        ...
        if not table:
            raise EroareAutentificareHidroelectrica(
    ...
    async def _post(
        ...
        try:
            ...
            raise EroareApiHidroelectrica(f"{label}: Timeout") from exc
        except EroareApiHidroelectrica:
            raise
        except Exception as exc:
            _LOGGER.error("[%s] Eroare: %s", label, exc)
            raise EroareApiHidroelectrica(f"{label}: {exc}") from exc

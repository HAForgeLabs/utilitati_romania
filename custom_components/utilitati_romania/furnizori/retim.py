...
URL_BAZA = "https://cp.retim.ro"
URL_API = f"{URL_BAZA}/api"
ORIGIN = "https://retim.ro"
...
class EroareApiRetim(Exception):
    pass


class EroareAutentificareRetim(EroareApiRetim):
    pass


class EroareConectareRetim(EroareApiRetim):
    pass


class EroareRaspunsRetim(EroareApiRetim):
    pass


class ClientApiRetim:
        ...
        try:
            async with self._sesiune.request(
                ...
                if raspuns.status in (401, 403, 419):
                    ...
                    raise EroareAutentificareRetim(f"Autentificare RETIM eșuată pentru {endpoint}: HTTP {raspuns.status}")
                if raspuns.status >= 400:
                    raise EroareApiRetim(f"RETIM a returnat HTTP {raspuns.status} pentru {endpoint}: {text[:500]}")
                if not text.strip():
                    return {}
                try:
                    return await raspuns.json(content_type=None)
                except Exception as err:
                    raise EroareRaspunsRetim(f"Răspuns JSON invalid pentru {endpoint}: {text[:500]}") from err
        except EroareApiRetim:
            raise
        except aiohttp.ClientError as err:
            raise EroareConectareRetim(f"Eroare de conectare la RETIM pentru {endpoint}: {err}") from err
        except TimeoutError as err:
            raise EroareConectareRetim(f"Timeout la RETIM pentru {endpoint}") from err
    ...
    async def async_login(self) -> dict[str, Any]:
        ...
        if not isinstance(raspuns, dict):
            raise EroareAutentificareRetim("Login RETIM a returnat un răspuns invalid")
        ...
        succes = raspuns.get("success", raspuns.get("status"))
        if succes is False or any(cuv in mesaj for cuv in ("invalid", "wrong", "incorrect", "eroare", "parol")):
            raise EroareAutentificareRetim("Credentialele RETIM nu au fost acceptate")
    ...
    async def async_get_docs(self, endpoint: str, *, optional: bool = False) -> list[dict[str, Any]]:
        try:
            return _extrage_lista(await self._request("GET", endpoint))
        except EroareApiRetim:
            if not optional:
                raise
            _LOGGER.debug("Endpointul RETIM %s nu este disponibil pentru acest cont", endpoint, exc_info=True)
            return []
    ...
    async def async_sync_customers(self) -> None:
        try:
            ...
            _LOGGER.debug("Sincronizarea clienților RETIM nu a reușit", exc_info=True)
    ...
    async def async_get_all_data(self) -> dict[str, Any]:
        ...
        return {
            ...
            "card_payments": await self.async_get_docs(ENDPOINT_PLATI_CARD, optional=True),
            "payments": await self.async_get_docs(ENDPOINT_PLATI, optional=True),
            "update_requests": await self.async_get_docs(ENDPOINT_CERERI_ACTUALIZARE, optional=True),
            "orders": await self.async_get_docs(ENDPOINT_COMENZI, optional=True),
            "products": await self.async_get_docs(ENDPOINT_PRODUSE, optional=True),
        }


class ClientFurnizorRetim(ClientFurnizor):
    cheie_furnizor = "retim"
    nume_prietenos = "RETIM"
    ...
    def __init__(self, *, sesiune, utilizator: str, parola: str, optiuni: dict) -> None:
        ...
        self.api = ClientApiRetim(sesiune=sesiune, email=utilizator, parola=parola)

    async def async_testeaza_conexiunea(self) -> str:
        try:
            rezultat = await self.api.async_validate_credentials()
        except EroareAutentificareRetim as err:
            raise EroareAutentificare(str(err)) from err
        except EroareConectareRetim as err:
            raise EroareConectare(str(err)) from err
        except EroareRaspunsRetim as err:
        ...
        unic = _primul_text(client, "custAccount", "customer_code", "cod_client", "id", "customer_id", "customerId") or _primul_text(user, "id", "email", adanc=True)
    ...
    def _mapeaza_facturi(self, date_brute: dict[str, Any], conturi: list[ContUtilitate]) -> list[FacturaUtilitate]:
        ...
        for factura in date_brute.get("invoices", []) or []:
            ...
            rest_plata = _float_sigur(_primul_valoare(factura, "dueAmount", "due_amount", "remaining", "amount_remaining", "amountRemaining", "rest", "rest_plata", "unpaid_amount", "unpaidAmount", "amount_to_pay", "amountToPay"))
...
def _adresa_din_obiect(obiect: dict[str, Any]) -> str | None:
    addresses = obiect.get("addresses")
    if isinstance(addresses, list):
        for item in addresses:
            if isinstance(item, dict):
                direct_list = _primul_text(item, "address", "adresa", "full_address", "fullAddress")
                if direct_list:
                    return direct_list
...
def _data_text(obiect: dict[str, Any]) -> str | None:
    data = _data_sigura(_primul_valoare(obiect, "transactionDate", "transaction_date", "date", "created_at", "createdAt", "paid_at", "paidAt", "payment_date", "paymentDate"))
...
def _ultima_inregistrare_dupa_data(items: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    if not valid:
        return None
    return sorted(valid, key=lambda item: _data_sigura(_primul_valoare(item, "transactionDate", "transaction_date", "date", "created_at", "createdAt", "paid_at", "paidAt", "payment_date", "paymentDate")) or date.min, reverse=True)[0]
...
def _deduce_stare_factura(factura: dict[str, Any], rest_plata: float | None) -> str:
    status = str(_primul_text(factura, "statusName", "status_name", "payment_status", "paymentStatus", "state", "stare", "status") or "").strip().lower()
